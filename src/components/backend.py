...
def generate_questions(text, num_questions, max_tokens=512, overlap=50, mode="easy"):
    """
    Generate questions using the T5 model with difficulty control
    """
    ...
    # Add difficulty prefix to text
    prefix_map = {
        "easy": "generate simple question: ",
        "medium": "generate question: ",
        "hard": "generate detailed question: "
    }
    ...
    while start < len(tokens) and len(questions) < num_questions:
        ...
        # Prepend difficulty prefix
        prompt_text = prefix_map[mode] + chunk_text
        ...
        if len(encoded_with_special) <= max_tokens:
            results = t5_pipeline(
                ...
                max_length=max_tokens,
                truncation=True,
                num_return_sequences=2 if mode == "hard" else 1
            )
...
def generate_quiz(text, num_questions):
    ...
    try:
        file_path = os.path.abspath("Quizzes.json")
        print(f"Saving quiz to: {file_path}")  # Log the path
...
def handle_quiz():
    ...
    try:
        ...
        if result_status == -1:
            print("Quiz generation failed - unable to save results")
            ...
            return jsonify({"quiz": "No questions could be generated", "questions": []}), 500
...
def generate_questions_endpoint():
    ...
    if not mode or mode not in valid_modes:
        return jsonify({
            'error': '500 Insufficient parameters for the request. Required parameters: mode (easy/medium/hard)'
        }), 500
    pdf_path = "temp.pdf"
    try:
        file.save(pdf_path)
        ...
        generated_data = generate_questions(document_text, num_questions, mode=mode)
        if not generated_data:
            return jsonify({'error': 'Failed to generate questions'}), 500
        return jsonify({'status': 'success', 'questions': generated_data}), 200
    except Exception as e:
